...
- **PWA:** `vite-plugin-pwa` — manifest, icons, service worker with stale-while-revalidate for the app shell and network-first for API calls; cached last-good readings shown with a staleness banner when offline. Age is read off the payload's newest hour, never off the fetch: a service-worker cache hit arrives looking live and can carry six-hour-old air. With no readings at all, the quick log still saves the rating and backfills its exposure vector from Open-Meteo's three-day history afterwards.
...
- **Hosting:** GitHub Pages via Actions workflow, on its own apex domain `breathingindex.com` (`base: '/'` in `vite.config.ts`; `public/CNAME` carries the custom domain into every deploy artifact). It started as a project site under the drewhoover.com user site at `/breathing-index/`. Setting the custom domain makes GitHub Pages 301 that entire prefix to the new domain with the path preserved, so old links need no help from the `DrewHoo.github.io` repo — and conversely, nothing can be served under that prefix any more.
...
7. **M7 — Own domain:** ✅ done (2026-08-07) — moved to https://breathingindex.com/ (`base: '/'`, `public/CNAME`, absolute OG/Twitter URLs). Two consequences of the move are worth remembering, because neither can be fixed from the server side:
   ...
   - **`localStorage` is per-origin, so diary data does not follow the move.** It has to travel by the settings-screen export/import. Usefully, the frozen old install is what keeps the export reachable — so export before deleting it. The stored value under `breathing-index.diary.v1` is byte-identical to the export format, so a devtools copy of that key is an equivalent fallback.

## Open questions

- **Exposure windows per variable:** one window per mechanism, settled for now by [specs/22-exposure-windows.md](specs/22-exposure-windows.md) — ozone on an 8-hour mean and PM on a 24-hour one, each matching the averaging its own published breakpoints are written at; grass pollen on the highest of three local days, the shape of the only pollen-and-asthma signal worth trusting; the weather stresses on the hour they were breathed. PM10 is computed on the same 24-hour mean and shown on its row without being graded ([24](specs/24-vector-diet.md)). The table lives in [docs/trigger-model.md](docs/trigger-model.md). Still open is the tuning: these are the literature's windows, not this user's. Retuning is not free — bounds are learned against features, so the source name carries a window generation (`cams-w2`) and changing a window retires everything learned under the old one.
- **How many variables is too many?** Each added dimension slows attribution (bigger candidate sets, and correlated pairs like heat+ozone rarely decorrelate naturally). Keep the vector mechanistically plausible for the user; let empty-candidate-set conflicts drive additions. Subtraction counts too: [24](specs/24-vector-diet.md) cut PM10 to display-only and dropped NO₂ entirely, on the grounds that neither could ever be told apart from what was already in the vector.
