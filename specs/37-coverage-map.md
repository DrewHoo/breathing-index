...
- **Italy is the clean case.** Measured (POLLnet), an openly licensed model (CAMS via Open-Meteo covers Europe), and the calendar, all three over the same days. This is where "the model matches the calendar" gets its strongest number. **Amended 2026-09-17 (as built):** POLLnet's geoserver was unreachable all day, so the measured side is Arpae Emilia-Romagna's open CSV feed instead — 13 stations, daily counts by family, CC-BY, current within days, with a 2013-onward archive that supplies the calendar as a day-of-year climatology. Region average rather than per station, because the feed's station registry was deleted upstream and ids cannot be honestly paired with cities; the model side averages over the network's 13 documented cities, and a mean needs no pairing. First run: the model and the calendar finish within a few hundredths on every pollen, the calendar wins on mugwort, Alternaria's calendar is near-useless (0.10) while yesterday's count scores 0.74, and the PM2.5 control shows the model only matching persistence once no seasonal curve does the work.
